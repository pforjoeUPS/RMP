...
def calculating_diff_z_score(data,underlier_lambda,z_score_lambda):
    ...
    diff = 0
    prev_diff_roll_mean = 0
    prev_diff_roll_var = 0
    prev_diff_roll_stdev = 0
    diff_roll_mean = 0
    diff_roll_var = 0
    ...
    for index, row in underlying_data_SPX_VIX.iterrows():
        ...
        if skip_first:
            ...
            ret_roll_var = underlier_lambda*(prev_ret_roll_var+(ret_roll_mean-prev_ret_roll_mean)**2)+(1-underlier_lambda)*(daily_ret-ret_roll_mean)**2
            ...
            if hist_vol < 0:    
                ...
                ihist_vol =(((prev_hist_vol/(100*252**0.5))**2 + (prev_ret_roll_mean-iret_roll_mean)**2)*underlier_lambda + (1-underlier_lambda)*(intrdaily_ret-iret_roll_mean)**2)**0.5*252**0.5*100
            #return
            prev_spx = row['SPX']        
            ...
            if prev_diff_roll_mean == 0:
                ...
                diff_roll_var = z_score_lambda*(prev_diff_roll_var+(diff_roll_mean-prev_diff_roll_mean)**2) + (1-z_score_lambda)*(diff-diff_roll_mean)**2
                ...
                idiff_roll_stdev =((prev_diff_roll_stdev**2+(prev_diff_roll_mean-idiff_roll_mean)**2)*z_score_lambda + (1-z_score_lambda)*(idiff-idiff_roll_mean)**2)**0.5

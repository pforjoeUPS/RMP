...
from ..datamanager.data_xformer_new import get_price_data
...
def get_up_down_capture_ratio(return_series, mkt_series, upside=True):
    ...
    ret_cap = dm.merge_dfs(mkt_cap, return_series, True)[return_series.name]
...
def get_max_dd(return_series):
    ...
    price_series = get_price_data(return_series)
...
def get_port_analytics(return_series, freq='M', rfr=0.0, target=0.0, p=0.05):
    ...
    ret_vol = get_sharpe_ratio(return_series, freq, rfr)
    sortino = get_sortino_ratio(return_series, freq, target, rfr)
    ...
    if empty:
        ...
        try:
            ...
            return {'bmk_name': bmk_name, 'bmk_beta': beta_bmk, 'excess_ret': excess_ret,
                    'te': te, 'downside_te': dwnside_te, 'te_downside_te': te / dwnside_te, 'ir': ir, 'ir_asym': ir_asym}
        except KeyError:

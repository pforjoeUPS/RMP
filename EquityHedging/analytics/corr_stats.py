...
def get_corr_analysis(df_returns, notional_weights=[], include_fi=False, weighted=False, include_bmk=False):
    """
    ...
    corr_dict = {
        ...
        "equity_up_down": [get_up_lwr_corr(equity_up, equity_down), get_title_string(strategy_returns, data_range,equity_id)]
        }
    ...
    if include_fi:
        ...
        corr_dict["fi_up_down"]= [get_up_lwr_corr(fi_up, fi_down), get_title_string(strategy_returns, data_range,fi_id)]
        
    
    if include_bmk:
        pass
        # bmk_id = col_list[2]
        # bmk_up = (strategy_returns[strategy_returns[bmk_id] > 0])
        # bmk_down = (strategy_returns[strategy_returns[bmk_id] < 0])
        # corr_dict["bmk_up"] = [bmk_up.corr(), get_title_string(bmk_up, data_range,bmk_id,'index_up')]
        # corr_dict["bmk_down"] = [bmk_down.corr(), get_title_string(bmk_up, data_range,bmk_id,'index_down')]

    return corr_dict

def get_corr_analysis1(df_returns, notional_weights=[], include_eq=False,include_fi=False, weighted=False):
    """
    Returns a dict of correlation matrices: 
        full correlation, when equity up, when equity down
    ...
    corr_dict = {"full": [strategy_returns.corr(), 
                          f'Correlation of {len(strategy_returns)} Historical Observations ({get_data_range(strategy_returns)})']}
    if include_eq:
        equity_id = col_list[0]
...
def get_conditional_corr(df_returns, strat_id):
    ...
    return [get_up_lwr_corr(strat_up, strat_down),  f'Conditional correlation where {strat_id} > 0 (upper) and < 0 (lower)']

...
def get_peak_loss(df_prices, col, look_back_days, day):
    """
    Returns peak return for a specific day during look back period
    ...
    # get minimum index level within the look back days period
    max_index_level = df_prices.loc[df_prices.index[day-look_back_days]:df_prices.index[day-1], col].max()

    #compute peak return
    return day_index_level/max_index_level
...
def get_decay_days(df_returns, col, freq="1M", retrace_pct=.50, sd =1.28, look_back_days=60, look_fwd_days=60):
    ...
    while day < len(df_prices):
        ...
        peak_loss = get_peak_loss(df_prices, col, look_back_days, day)

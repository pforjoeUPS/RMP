...
def transform_bbg_data(filename, sheet_name='data'):
    bbg_df = pd.read_excel(RETURNS_DATA_FP + filename, sheet_name=sheet_name, 
                           index_col=0,skiprows=[0,1,2,4,5,6])
    bbg_df.index.names = ['Dates']
    return bbg_df

def get_liq_alts_bmks(equity = 'M1WD',include_fi=True):
    bmks_index = transform_bbg_data('liq_alts\\liq_alts_bmks.xlsx')
...
def get_liq_alts_returns(equity='M1WD', include_fi=True):
    ...
    return merge_data_frames(liq_alts_bmks, liq_alts_port)
    
def get_liq_alts_port(liq_alts_dict):
    liq_alts_port = pd.DataFrame()
    for key in liq_alts_dict.keys:
        liq_alts_port = merge_data_frames(liq_alts_port, liq_alts_dict[key]['returns'], drop_na=False)
        if key == 'Total Liquid Alts':
            liq_alts_port = merge_data_frames(liq_alts_port, liq_alts_dict[key]['returns'][[key]],drop_na=False)

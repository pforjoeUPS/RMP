...
# Add KAF data
kaf_data = dm.get_new_strat_data('liq_alts\\kaf.xlsx')
kaf_co = kaf_data[['KAF-Commingled']]
kaf_sma = kaf_data[['KAF SMA']]
dd.get_worst_drawdowns(kaf_co)
dd.get_worst_drawdowns(kaf_sma)

# Add HF bmks
hf_bmks = dm.get_new_strat_data('liq_alts\\mkts_hfs.xlsx', index_data=True)
kaf_full = dm.merge_data_frames(liq_alts_p.bmk_returns['Monthly'], kaf_data, False)
kaf_full = dm.merge_data_frames(kaf_full, hf_bmks, True)

# Run report
kaf_dict = dm.get_monthly_dict(kaf_full)
rp.get_alts_report('kaf_full',kaf_dict)

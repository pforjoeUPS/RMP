...
# Add HF bmks
kaf_full = dm.merge_dfs(liq_alts_p.mkt_returns, kaf)
kaf_full = dm.merge_dfs(kaf_full, wb_ms, False)
kaf_full = dm.merge_dfs(kaf_full, voleon, False)
kaf_full = dm.merge_dfs(kaf_full, liq_alts_p.hf_returns, False)
kaf_full.dropna(inplace=True)
kaf_full.drop(['Commo Carry'], axis=1, inplace=True)

# Run report
kaf_dict = dm.get_period_dict(kaf_full)
rp.get_alts_report('KAF-test',kaf_dict)
...
for col in rolling_excess.columns:
    ...
    df_port_bmk = dm.merge_dfs(kaf_full[[col]], liq_alts_p.bmk_returns[[bmk_name]])
...
for col in corr_df:
    ...
    for x in temp_strat_list:
        rolling_corr_temp = dm.merge_dfs(rolling_corr_temp, get_rolling_corr(corr_df[col], corr_df[x], window), False)
    rolling_corr_temp.drop([col], axis=1, inplace=True)
...
return_dict = {}
df_returns = dm.merge_dfs(liq_alts_p.bmk_returns['Monthly'], liq_alts_p.sub_ports['Global Macro']['bmk_returns'], False)
df_returns = df_returns.iloc[97:,]
return_dict['Global Macro'] = dm.get_monthly_dict(df_returns)
df_returns = dm.merge_dfs(liq_alts_p.bmk_returns['Monthly'], liq_alts_p.sub_ports['Total Liquid Alts']['bmk_returns'], False)
df_returns = df_returns.iloc[104:,]
return_dict['Total Liquid Alts'] = dm.get_monthly_dict(df_returns)
df_returns = dm.merge_dfs(liq_alts_p.bmk_returns['Monthly'], liq_alts_p.returns, False)
...
df_returns_gm = dm.merge_dfs(liq_alts_p.mkt_returns, liq_alts_p.sub_ports['Global Macro']['bmk_returns'], False)
# df_returns_gm = dm.merge_data_frames(liq_alts_p.mkt_returns, liq_alts_p.bmk_returns['HFRX Macro/CTA'], False)
# df_returns_gm = liq_alts_p.bmk_returns.copy()
df_returns_gm.drop(['1907 Penso Class A','JSC Vantage', 'Capula TM Fund', 'KAF'], axis=1, inplace=True)
df_returns_gm = dm.merge_dfs(df_returns_gm, penso, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, jsc, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, tmf, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, kaf, False)
# df_returns_gm = dm.merge_data_frames(df_returns_gm,hfrxsvd_aifa, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, edl_c, False)
...
df_returns_gm = dm.merge_dfs(df_returns_gm, dymon_asia, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, gm_qm_a, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, gmo_sys, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, wellington[['WMSF Composite1']], False)
df_returns_gm = dm.merge_dfs(df_returns_gm, astignes, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, campbell_ar, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, modular, False)
df_returns_gm = dm.merge_dfs(df_returns_gm, liq_alts_p.sub_ports['Trend Following']['bmk_returns'], False)
df_returns_gm.drop(['Trend Following', '1907 ARP TF'], axis=1, inplace=True)
df_returns_gm = dm.merge_dfs(df_returns_gm, liq_alts_p.sub_ports['Total Liquid Alts']['bmk_returns'], False)
...
import copy
bmk_dict = copy.deepcopy(liq_alts_p.bmk_key)
...
df_ret_tf = dm.merge_dfs(liq_alts_p.mkt_returns, liq_alts_p.bmk_returns['SG Trend'], False)
df_ret_tf = dm.merge_dfs(df_ret_tf, liq_alts_p.sub_ports['Trend Following']['bmk_returns'], False)
...
df_returns_tf = dm.merge_dfs(liq_alts_p.mkt_returns, liq_alts_p.sub_ports['Trend Following']['bmk_returns'], False)
df_returns_tf.drop(['Trend Following'], axis=1, inplace=True)
df_returns_tf = dm.merge_dfs(df_returns_tf, welton_tf_strats, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, welton_tf_hypo, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, aqr_tf, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, aspect, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, liq_alts_p.sub_ports['Total Liquid Alts']['bmk_returns'], False)
...
df_returns_ar = dm.merge_dfs(liq_alts_p.mkt_returns, liq_alts_p.sub_ports['Absolute Return']['bmk_returns'], False)
df_returns_ar = dm.merge_dfs(df_returns_ar, gresham, False)
df_returns_ar = dm.merge_dfs(df_returns_ar, newton_carry, False)
df_returns_ar = dm.merge_dfs(df_returns_ar, rathmore, False)
df_returns_ar = dm.merge_dfs(df_returns_ar, tcc, False)

df_returns_ar = dm.merge_dfs(df_returns_ar, liq_alts_p.sub_ports['Total Liquid Alts']['bmk_returns'], False)
...
df_returns_ar = df_returns_ar[:-2]
df_returns_ar = df_returns_ar.iloc[85:,]
...
rp.get_alts_report('Gresham-fit',ret_dict_ar,include_bmk=True, bmk_df=liq_alts_p.bmk_returns, bmk_dict=bmk_dict)
...
df_returns_tf = dm.merge_dfs(liq_alts_p.mkt_returns, liq_alts_p.sub_ports['Trend Following']['bmk_returns'], False)
df_returns_tf.drop(['Trend Following'], axis=1, inplace=True)
df_returns_tf = dm.merge_dfs(df_returns_tf, welton_tf, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, transtrend, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, aspect, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, aqr_tf, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, one_river_alt, False)
df_returns_tf = dm.merge_dfs(df_returns_tf, liq_alts_p.sub_ports['Total Liquid Alts']['bmk_returns'], False)

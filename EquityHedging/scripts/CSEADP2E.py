...
from EquityHedging.reporting.excel import sheets, reports
import os
...
new_ret_dict['Monthly'] = monthly_ret
...
new_ret_dict['Quarterly'] = quarter_ret
...
new_ret_dict['Weekly'] = weekly_ret
...
new_ret_dict['Yearly'] = yearly_ret
...
reports.get_returns_report('returns_data', new_ret_dict)

#example of method
def create_report(report_name, returns_dict):
    writer= pd.ExcelWriter(report_name+'.xlsx',engine='xlsxwriter')
    #create new dictionary to be saved to returns data
    #new_ret_dict={"Daily":daily_ret, "Weekly":weekly_ret, "Monthly":monthly_ret,"Quarterly":quarter_ret,"Yearly":yearly_ret}

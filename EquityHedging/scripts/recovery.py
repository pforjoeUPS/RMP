# -*- coding: utf-8 -*-
"""
Created on Sun Sep 12 23:22:43 2021
...
def get_du_list(price_series, freq='1W', look_fwd_days=60, look_fwd_days_2=120):
    """
    ...
    # threshold = 1.28*rs.get_ann_vol(df_returns[col], freq)
...
def get_decay_retrace_list2(du,du_range,price_series, days):
    ...
    index_list = get_index_list(price_series, start, end)
    min_index = get_min_index(price_series, start, end, 0)
    peak = price_series[du['end']]
    trough = price_series[min_index]
    retrace_list=[]
    for day in days:
...
def compute_recovery_pct(price_series, freq='1W', look_fwd_days=60, look_fwd_days_2=120, dd_range = 260,days=[20,60,120,180,240]):
    ...
    return np.mean(recovery_list)

def compute_decay_pct(price_series, freq='1W', look_fwd_days=60, look_fwd_days_2=120, du_range = 260,days=[20,60,120,180,240]):
    """
    ...
    """
    #get drawup list
    du_list = get_du_list(price_series,freq,look_fwd_days, look_fwd_days_2)
    du_list = check_dd_du_list(du_list, price_series, du_range)
    ...
    for du in du_list:
        ...
        retrace_list = get_decay_retrace_list2(du, du_range, price_series,days)

...
equity_bmk = 'SPTR'
strat_drop_list = ['SPTR', 'Down Var', 'Vortex', 'VOLA', 'Dynamic Put Spread', 'VRR 2', 'VRR Trend', 'Corr Hedge', 'Def Var', 'Commodity Basket', 'ESPRSO', 'EVolCon', 'Moments']
include_fi = False
...
new_strat = pd.read_excel(dm.NEW_DATA + 'UBS_SKUE.xlsx',
                                           sheet_name = 'Sheet1', index_col=0)
new_strat_dict = dm.get_data_dict(new_strat)


new_strat = pd.read_excel(dm.NEW_DATA + 'Commods TICKERS.xlsx',
                                           sheet_name = 'MXWDIM', index_col=0)
new_strat_dict1 = dm.get_data_dict(new_strat)

#merge dictionaries
full_data_dict = dm.merge_dicts_list([new_strat_dict1, returns, new_strat_dict])

strat_report_name = 'UBCS GWDispersion and Skew.xlsx'
selloffs = True

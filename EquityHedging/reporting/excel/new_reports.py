...
class ReturnsReport(SetReport):
    ...
    def generate_report(self, *kwargs):
        ...
        for data_key in self.data:
            ...
            new_sheets.HistReturnSheet(writer=self.writer, returns_df=self.data[data_key],
                                       sheet_name=data_key[:diff]).create_sheet()
...
class ReturnMktValueReport(ReturnsReport):
    ...
    def generate_report(self):
        ...
        new_sheets.HistReturnSheet(writer=self.writer, returns_df=self.data['returns'],
...
class HistSelloffReport(ReturnsReport):
    def __init__(self, report_name, data_handler, weighted=False, new_strat=False):
        ...
        self.data = self.get_report_analytic_data()

        super().__init__(report_name=report_name, data=self.data, data_file=False)

    def get_report_analytic_data(self):
...
class EquityHedgeReport(HistSelloffReport):
    def __init__(self, report_name, data_handler, new_strat=False, weighted=True, selloffs=True,
    ...
    def get_report_analytic(self, data_handler):
        return summary_new.EqHedgeReportAnalytic(data_handler=data_handler, new_strat=self.new_strat,
                                                 weighted=self.weighted, selloffs=self.selloffs,
                                                 include_quantile=self.include_quantile)
    ...
    def generate_quantile_sheet(self):
        for mkt in self.data['quantiles']:
            new_sheets.QuantileDataSheet(self.writer, self.data['quantiles'][mkt],
...
class StratReport(EquityHedgeReport):
    def __init__(self, report_name, data_handler, selloffs=True):
        """
        ...
        super().__init__(report_name=report_name, data_handler=data_handler, new_strat=False,
...
class RollingCumRetReport(ReturnsReport):
    ...
    # self.rolling_cum_dict = summary.get_rolling_cum_data(self.data_dict['Returns'], self.freq,
    #                                                      self.notional_weights)
    #
    # # Create Excel reports
    # new_sheets.HistReturnSheet(self.writer, self.rolling_cum_dict['3M'],
    #                            'Rolling Cum Returns_3 Months').create_sheets()
    # new_sheets.HistReturnSheet(self.writer, self.rolling_cum_dict['6M'],
    #                            'Rolling Cum Returns_6 Months').create_sheets()
    # new_sheets.HistReturnSheet(self.writer, self.rolling_cum_dict['1Y'],
    #                            'Rolling Cum Returns_1 Year').create_sheets()


class AltsReport(ReturnsReport):
    pass
    # def __init__(self, report_name, returns_data, mv_data={}, include_fi=True,
    #              freq='1M', include_bmk=False, bmk_df=pd.DataFrame(), bmk_dict={}):
    #
